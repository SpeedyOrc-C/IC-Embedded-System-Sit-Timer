<script lang="ts">
    ...
    async function LogOutFromAllPlaces()
    {
        ...
        try
        {
            await fetch("/api/log-out/all-places")
        } finally
        {
            waiting = false
            await goto("/log-in")
            ...
            {#each data.devices as {name, device_id}, index}
                <article>
                    ...
                    <div class="operations">
                        ...
                        <button title="Change name" class="btn-change-name" onclick={() => ChangeDeviceName(index, name)} disabled={waiting}>
                        ...
                        <button title="Copy ID" class="btn-copy-id" onclick={() => CopyId(device_id)}>
                        ...
                        <button title="Delete" class="btn-delete" onclick={() => DeleteDevice(index)} disabled={waiting}>
...
<style>
    ...
    article {
        min-width: 18rem;
        width: fit-content;
        box-shadow: 0 0 0 1px #333, 4px 4px 0 1px #0003;
        margin: 1rem;
        padding: 1rem;
